...
huffman_tree::huffman_tree(const huffman_tree &other) { copy(other); }
...
void huffman_tree::build_tree(const vector<frequency> &frequencies) {
  ...
  for (auto f : frequencies) {
    single_queue.emplace(std::make_unique<node>(f));
  }

  while (single_queue.size() + merge_queue.size() > 1) {

    auto left_child = remove_smallest(single_queue, merge_queue);
    auto right_child = remove_smallest(single_queue, merge_queue);
    int sum_freq = left_child->freq.count() + right_child->freq.count();
    auto internal_node = std::make_unique<node>(std::move(sum_freq));
    internal_node->left = std::move(left_child);
    internal_node->right = std::move(right_child);
    merge_queue.emplace(std::move(internal_node));

    if (single_queue.size() + merge_queue.size() == 1) {
...
auto huffman_tree::remove_smallest(
  ...
  if (merge_queue.empty()) {
    auto smallest = std::move(single_queue.front());
    single_queue.pop();
    return smallest;
  }
  if (single_queue.empty()) { 

    auto smallest = std::move(merge_queue.front());
    merge_queue.pop();
    return smallest;
  }
...
vector<bool> huffman_tree::bits_for_char(char c) { return bits_map_[c]; }
...
auto huffman_tree::read_tree(binary_file_reader &bfile)
    -> std::unique_ptr<node> {
  ...
  if (bfile.next_bit()) {
    auto leaf_node = std::make_unique<node>(frequency(bfile.next_byte(), 1));
    return leaf_node;
  } else {
    auto internal_node = std::make_unique<node>(0);
    internal_node->left = read_tree(bfile);

    internal_node->right = read_tree(bfile);
...
public:
  huffman_tree_node_descriptor(const node *root) : subroot(root) { /* nothing */
  }
  ...
  bool isNull() const { return !subroot; }
